...
type RegistryItem = {
  key: string;
  label: string;
  Comp: React.ComponentType<HeaderProps>;
  defaultProps: HeaderProps;
  previewProps?: HeaderProps;
};
...
/** ---------- Auto-discover headers from parent folder (../Header*.tsx) ---------- */
type HeaderModule = {
  default: React.ComponentType<HeaderProps>;
  label?: string;
  defaultProps?: HeaderProps;
  previewProps?: HeaderProps;
};
...
// Extract numeric index for natural sort (Header2 < Header10)
const headerIndex = (fileKey: string) => {
  const clean = fileKey.replace(/^\.\/|(\.tsx|\.jsx|\.ts|\.js)$/g, ""); // "Header12"
  const m = clean.match(/Header\s*(\d+)/i);
  return m ? parseInt(m[1], 10) : Number.MAX_SAFE_INTEGER;
};

function useRegistry(): RegistryItem[] {
  const ctx = useMemo(() => require.context("..", false, /^\.\/Header.*\.(t|j)sx$/), []);
  return useMemo<RegistryItem[]>(() => {
    const keys: string[] = ctx.keys().sort((a, b) => {
      const na = headerIndex(a), nb = headerIndex(b);
      return na === nb ? a.localeCompare(b) : na - nb;
    });
    return keys.map((k) => {
      const mod: HeaderModule = ctx(k);
...
/** ---------- Grid sample data ---------- */
const makeGridData = (base: HeaderProps): HeaderProps[] => {
  const ids = [
...
const ALL_KEY = "__ALL__"; // NEW: special key for "show all"
...
const FloatingLibrary: React.FC<FloatingLibraryProps> = ({
  ...
  const augmented = useMemo(() => [{ key: ALL_KEY, label: "All Headers" } as any].concat(registry), [registry]);
  ...
  const prevHeader = () => { if (!total) return; setSelectedKey(augmented[(selectedIndex - 1 + total) % total].key); };
  const nextHeader = () => { if (!total) return; setSelectedKey(augmented[(selectedIndex + 1) % total].key); };

  return (
    <div className="fixed z-50" style={{ left: pos.x, top: pos.y, width: minimized ? 260 : 320 }} role="dialog" aria-label="Header Library">
      {/* Header */}
      <div onMouseDown={onMouseDownHeader} className="cursor-move select-none flex items-center justify-between rounded-t-2xl border border-b-0 border-neutral-700 bg-neutral-800 px-3 py-2 shadow">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-neutral-100">Header Library</span>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={prevHeader} className="rounded-lg border border-neutral-600 bg-neutral-700 p-1.5 text-neutral-100 hover:bg-neutral-600" aria-label="Previous"><ChevronLeft className="h-4 w-4" /></button>
          <span className="mx-1 text-xs tabular-nums text-neutral-200">{total ? selectedIndex + 1 : 0} / {total}</span>
          <button onClick={nextHeader} className="rounded-lg border border-neutral-600 bg-neutral-700 p-1.5 text-neutral-100 hover:bg-neutral-600" aria-label="Next"><ChevronRight className="h-4 w-4" /></button>
        ...
        <div className="rounded-b-2xl border border-neutral-700 bg-neutral-900 shadow-lg overflow-hidden">
            ...
            <div className="grid grid-cols-5 gap-2">
              {(["auto", "sm", "md", "lg", "xl"] as ViewMode[]).map(mode => (
                <button
                  ...
                  className={`h-8 rounded-lg border text-xs uppercase ${viewMode === mode ? "border-neutral-300 bg-neutral-200 text-neutral-900"
          ...
          <div ref={listRef} className="relative overflow-y-auto pr-2" style={{ maxHeight: "50vh" }}>
            <div className="space-y-3 px-3 py-3">
              ...
              <div
                data-item
                className={`cursor-pointer rounded-xl border bg-neutral-800 text-neutral-100 transition-shadow ${selectedKey === ALL_KEY ? "ring-2 ring-neutral-200 border-neutral-600 shadow" : "ring-1 ring-black/30 border-neutral-700 hover:shadow"
                  }`}
              ...
              >
                ...
                <div className="p-3">
                  <p className="text-xs font-medium">All Headers</p>
              ...
              {registry.map((item) => {
                ...
                return (
                  <div
                    key={item.key}
                    data-item
                    className={`cursor-pointer rounded-xl border bg-neutral-800 text-neutral-100 transition-shadow ${active ? "ring-2 ring-neutral-200 border-neutral-600 shadow" : "ring-1 ring-black/30 border-neutral-700 hover:shadow"
                      }`}
          ...
          <div className="px-3 pb-3 text-[11px] text-neutral-400">
            Drag this window by the header. Click a header to load it — or use "All Headers" to render everything.
          </div>
        </div>
      )}
    </div>
...
/** ---------- Page (grid + viewport frame; now supports ALL view) ---------- */
export default function HeaderViewerPage() {
  const registry = useRegistry();
  const [selectedKey, setSelectedKey] = useState<string>(registry[0]?.key ?? ALL_KEY); // default to ALL if empty
    ...
    return (
      <div className="min-h-screen bg-neutral-100 p-8">
        <div className="mx-auto max-w-xl text-center bg-white rounded-2xl shadow-sm ring-1 ring-black/5 p-8">
          <h1 className="text-xl font-semibold mb-2">No headers discovered</h1>
          <p className="text-neutral-600 text-sm">
            Place files named <code>Header*.tsx</code> in <code>/Library/Headers/</code> next to <code>HeaderViewer</code>.
          </p>
        </div>
      </div>
    );
  ...
  // 1) make renderHeaderGrid accept `single` and switch data + grid class
  // REPLACE your renderHeaderGrid with this:
  const renderSingleHeader = (item: RegistryItem) => {
    const C = item.Comp;
...
<div className="mx-auto" style={{ width: frameWidth === "auto" ? "100%" : `${frameWidth}px` }}>
  <div className="mb-4 flex items-center justify-between">
    <h1 className="text-xl md:text-2xl font-semibold text-neutral-900">All Headers</h1>
    <div className="text-sm text-neutral-500">{registry.length} variants</div>
  </div>

  {/* One per row, no repetition */}
  <div>
    {registry.map(renderSingleHeader)}
  </div>
